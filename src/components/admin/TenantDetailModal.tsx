...
export default function TenantDetailModal({ tenantId, isOpen, onClose, onEdit }: TenantDetailModalProps) {
  ...
  const titleNode = (
    <div className="flex items-center gap-3">
      <span>{isLoading ? "Synchronizing Data..." : details?.tenant.name}</span>
      {!isLoading && details && (
  ...
  const footerNode = details ? (
    <div className="flex gap-4 w-full">
      ...
      >
        <ShieldAlert size={18} className="mr-2 inline" />
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title={titleNode}
      subtitle={details ? `Organization Intelligence • ${details.tenant.code}` : "Organization Intelligence"}
    ...
    >
      {isLoading ? (
        ...
        <>
          {/* Tabs Trigger */}
          <div className="-mt-8 -mx-8 px-8 pt-4 flex gap-8 shrink-0 border-b border-slate-50 mb-6 bg-slate-50/20">
