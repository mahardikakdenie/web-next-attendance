                ...
                <div className="text-center md:text-right flex-1 flex flex-col md:items-end w-full">
                  ...
                  <PDFDownloadLink
                    document={
                      <PayslipPDFDocument 
                        data={{
                          gross_income: selectedEmployeeSlip.breakdown?.earnings?.gross_income,
                          pph21_amount: selectedEmployeeSlip.breakdown?.deductions?.pph21_amount,
                          net_salary: selectedEmployeeSlip.net_salary,
                          total_deductions: selectedEmployeeSlip.breakdown?.deductions?.total_deductions,
                          total_employer_cost: selectedEmployeeSlip.breakdown?.employer_contributions?.total_employer_cost,
                          run_type: selectedEmployeeSlip.run_type,
                          method: selectedEmployeeSlip.method,
                          breakdown: {
                            prorated_basic: selectedEmployeeSlip.breakdown?.earnings?.basic_salary,
                            fixed_allowances: selectedEmployeeSlip.breakdown?.earnings?.fixed_allowances,
                            variable_allowances: selectedEmployeeSlip.breakdown?.earnings?.variable_allowances,
                            unpaid_leave_deduction: selectedEmployeeSlip.breakdown?.deductions?.unpaid_leave_deduction,
                            overtime_pay: selectedEmployeeSlip.breakdown?.earnings?.overtime_pay,
                            incentives: selectedEmployeeSlip.breakdown?.earnings?.incentives,
                            gross_income: selectedEmployeeSlip.breakdown?.earnings?.gross_income,
                            pph21_amount: selectedEmployeeSlip.breakdown?.deductions?.pph21_amount,
                            tax_allowance: selectedEmployeeSlip.breakdown?.earnings?.tax_allowance,
                            bpjs_allowance: selectedEmployeeSlip.breakdown?.earnings?.bpjs_allowance,
