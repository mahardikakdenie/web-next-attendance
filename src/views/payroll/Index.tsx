...
export default function PayrollView() {
    ...
    baseCols.push({
      ...
      accessor: (emp) => (
        <div className="flex flex-col gap-1.5">
          <Badge className={`${emp.status === "Published" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"} border-none text-[9px] font-black uppercase tracking-widest px-2.5 w-fit`}>
            {emp.status}
          </Badge>
          <div className="flex items-center gap-1">
             <Badge className={`${
               emp.run_type === 'THR' ? 'bg-emerald-100 text-emerald-700' : 
               emp.run_type === 'Bonus' ? 'bg-purple-100 text-purple-700' : 
               'bg-blue-100 text-blue-700'
             } border-none text-[8px] font-black uppercase tracking-tight px-1.5 py-0.5`}>
               {emp.run_type || 'Regular'}
             </Badge>
             <Badge className="bg-slate-100 text-slate-500 border-none text-[8px] font-black uppercase tracking-tight px-1.5 py-0.5">
               {emp.method || 'Gross'}
             </Badge>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-10">
      ...
      {/* 3 Main Tabs Integration */}
      <div className="flex p-1.5 bg-neutral-100 rounded-[22px] w-fit border border-neutral-200/50 shadow-inner mb-4">
        <button 
          onClick={() => setMainTab("bulk")}
          className={`flex items-center gap-2 px-6 py-2.5 rounded-[18px] text-[11px] font-black uppercase tracking-wider transition-all ${mainTab === "bulk" ? "bg-white text-blue-600 shadow-md" : "text-neutral-400 hover:text-neutral-900"}`}
        >
          <Layers size={16} />
        ...
        <button 
          ...
          className={`flex items-center gap-2 px-6 py-2.5 rounded-[18px] text-[11px] font-black uppercase tracking-wider transition-all ${mainTab === "calculator" ? "bg-white text-blue-600 shadow-md" : "text-neutral-400 hover:text-neutral-900"}`}
        >
          <Calculator size={16} />
          <span>Stateless Calculator</span>
        </button>
        <button 
          onClick={() => setMainTab("individual")}
          className={`flex items-center gap-2 px-6 py-2.5 rounded-[18px] text-[11px] font-black uppercase tracking-wider transition-all ${mainTab === "individual" ? "bg-white text-blue-600 shadow-md" : "text-neutral-400 hover:text-neutral-900"}`}
        >
          <UserPlus size={16} />
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <h1 className="text-3xl font-black text-neutral-900 tracking-tight leading-none">Payroll Dashboard</h1>
                <button 
                  onClick={toggleMask}
            ...
            <div className="flex gap-3">
              ...
              >
                ...
                <span className="font-bold">Generate Bulk</span>
              </Button>
            </div>
          </div>

          {/* Sub Tabs for HR/Finance view */}
